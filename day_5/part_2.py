...
if __name__ == "__main__":
    ...
    seed_info = re.findall(r"\d+\s\d+", lines[:1][0])
    ...
    for idx, item in enumerate(seed_info):
        print("processing item {} of {}".format(idx + 1, len(seed_info)))
        seed_range_start, seed_range_offset = [int(x) for x in item.split(" ")]

        start = seed_range_start
        end = seed_range_start + seed_range_offset

        while start < end:
            location = find_location_for_seed(seed_number=start)
